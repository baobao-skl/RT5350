...
//#endif
				...
				if (pAdapter->bWscDriverAutoUpdateCfg)
				{
	...
	if (pWscControl->WscState == WSC_STATE_CONFIGURED)
	{
			...
			)
		{
			...
			IF_DEV_CONFIG_OPMODE_ON_STA(pAdapter)
			{
				...
				{
					WscBuildBeaconIE(pAdapter, WSC_SCSTATE_CONFIGURED, FALSE, 0, 0, BSS0, NULL, 0);
					WscBuildProbeRespIE(pAdapter, WSC_MSGTYPE_REGISTRAR, WSC_SCSTATE_CONFIGURED, FALSE, 0, 0, BSS0, NULL, 0);
					WscLinkDown(pAdapter);
				}
//#ifdef KTHREAD_SUPPORT
//				WAKE_UP(&(pAdapter->wscTask));
	...
	if (pWscControl != NULL)
	{
		...
		if (pWscControl->Wsc2MinsTimerRunning)
		{
			...
			{				
				...
				{
					AsicDisableSync(pAd);
					WscBuildBeaconIE(pAd, pWscControl->WscConfStatus, FALSE, 0, 0, BSS0, NULL, 0);
...
//	UCHAR 			Data[256];
	...
	if (pWpsCtrl->WscV2Info.bEnableWpsV2)
	{
		...
		if (pAuthorizedMACs)
		{
			templen = AppendWSCTLV(WSC_ID_AUTHORIZED_MAC, pData, pAuthorizedMACs, AuthorizedMACsLen);
			pData += templen;
			Len   += templen;
		}
	}
			...
			case WSC_ID_AUTHORIZED_MAC:
				if (WscLen > 0)
				{
					...
					if (pAuthorizedMACs)
					{
						...
						NdisMoveMemory(pAuthorizedMACs, pData, WscLen);
    ...
    IN  PRTMP_ADAPTER pAd)
{
		...
		for (index = 0; index < pAd->ApCfg.BssidNum; index++)
		{
			if (pAd->ApCfg.MBSSID[index].SsidLen)
			{
				...
				if (index == 0)
				{
					...
					sprintf(profileTemp, "%s", profileTemp2);
				}
				else
				{
					sprintf(profileTemp3, "%s;%s", profileTemp, profileTemp2);
					sprintf(profileTemp, "%s", profileTemp3);
				}
			}
		}
	}
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		for (index = 0; index < pAd->ApCfg.BssidNum; index++)
		{
			...
			if (index == 0)
				sprintf(profileTemp, "%d", pAd->ApCfg.MBSSID[index].DefaultKeyId+1);
			else
				sprintf(profileTemp, "%s;%d", profileTemp, pAd->ApCfg.MBSSID[index].DefaultKeyId+1);
		}
	}
    ...
    IN ULONG Context)
{
	...
	while (pTask && !RTMP_OS_TASK_IS_KILLED(pTask))
	{
		...
		if (pAd->pWscElme && (pAd->pWscElme->MsgLen != 0))
		{
			...
			if (pElme)
			{
//				NdisZeroMemory(pElme->Msg, sizeof(MLME_QUEUE_ELEM));
