		...
		*/
		...
		if ((pAd->ApCfg.MBSSID[apidx].WscControl.WscConfMode & WSC_PROXY) != WSC_DISABLE)
		{
			...
			/*
				...
				4: Len of WSC OUI - 00 50 F2 04 
			*/
			bufLen = sizeof(WSC_IE_PROBREQ_DATA) + PeerWscIeLen + 4;
			...
			if(pBuf)
			{
				...
				if (32 >= *SsidLen)	//Well, I think that it must be TRUE!
				{
					...
					pprobreq->data[1] = PeerWscIeLen + 4;							// element Length					
					NdisMoveMemory((pBuf+sizeof(WSC_IE_PROBREQ_DATA)), WPS_OUI, 4);	// (WscProbeReqData)
					NdisMoveMemory((pBuf+sizeof(WSC_IE_PROBREQ_DATA)+4), pPeerWscIe, PeerWscIeLen);	// (WscProbeReqData)
