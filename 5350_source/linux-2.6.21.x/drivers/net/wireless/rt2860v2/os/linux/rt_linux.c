...
/* the lock will not be used in TX/RX path so throughput should not be impacted */
	...
	IN	UINT32				data_len)
{
	...
	MEM_DBG_PKT_ALLOC_INC(skb);
	SET_OS_PKT_NETDEV(skb, pNetDev); //get_netdev_from_bssid(pAd, apidx));
	...
	_announce_802_3_packet(pCtrlBkPtr, skb);
...
{   	
...
#endif // MONITOR_FLAG_11N_SNIFFER_SUPPORT //
	{
		...
		ph->rssi.status = 0;
		ph->rssi.len = 4;
	    ph->rssi.data = MaxRssi; //(u_int32_t)RTMPMaxRssiOrg(pAd, ConvertToRssiOrg(pAd, pRxBlk->pRxWI->RSSI0, RSSI_0), ConvertToRssiOrg(pAd, pRxBlk->pRxWI->RSSI1, RSSI_1), ConvertToRssiOrg(pAd, pRxBlk->pRxWI->RSSI2, RSSI_2));;
	...
	else
	{
	    ...
	    if (ShortGI == 1)
	  	...
	  	if (AMPDU)   // RXD_STRUC   PRT28XX_RXD_STRUC				pRxD = &(pRxBlk->RxD);
	  	...
	  	if (PHYMODE >= MODE_HTMIX)
	  	{
			if (MCS == 32)
			{
				if (ShortGI)
	     		...
	     		ph_11n33->data_rate = (16*4 + ((UCHAR)BW *16) + ((UCHAR)ShortGI *32) + ((UCHAR)MCS));                          
	     	else
	     		ph_11n33->data_rate = 16 + ((UCHAR)BW *16) + ((UCHAR)ShortGI *32) + ((UCHAR)MCS);
	  	}
 ...
 *******************************************************************************/
static inline NDIS_STATUS __RtmpOSTaskKill(
	IN OS_TASK *pTask)
{
//	RTMP_ADAPTER *pAd;
...
static inline INT __RtmpOSTaskNotifyToExit(
	IN OS_TASK *pTask)
{
...
static inline void __RtmpOSTaskCustomize(
	IN OS_TASK *pTask)
{
...
#if WIRELESS_EXT >= 12
	os_alloc_mem(NULL, (UCHAR **)ppIwStats,
	...
	IN	BOOLEAN					bSet)
{
	...
	if (bSet == TRUE)
	{
		os_alloc_mem(NULL, (UCHAR **)&(pOSFSInfoOrg->pContent), sizeof(OS_FS_INFO));
...
#ifndef WORKQUEUE_BH
	if (pTasklet->pContent != NULL)
		((OS_NET_TASK_STRUCT *)(pTasklet->pContent))->data = (ULONG)Data;
#endif // WORKQUEUE_BH //
}
	...
	IN	UINT32					RscLen)
{
	...
	if (pRsc->pContent == NULL)
	{
		...
		if (pRsc->pContent == NULL)
		{
			DBGPRINT(RT_DEBUG_ERROR, ("%s: alloc timer fail!\n", __FUNCTION__));
			return FALSE;
		}
		else
		{
...
{
	if (RTMP_OS_Alloc_Rsc(pTaskList, pTask,
							sizeof(OS_TASK)) == FALSE)
	{
		DBGPRINT(RT_DEBUG_ERROR, ("%s: alloc task fail!\n", __FUNCTION__));
		return FALSE; /* allocate fail */
	}
...
BOOLEAN RtmpOsPktOffsetInit(VOID)
{
	...
	if ((RTPktOffsetData == 0) && (RTPktOffsetLen == 0) && (RTPktOffsetCB == 0))
	{
		pPkt = kmalloc(sizeof(struct sk_buff), GFP_ATOMIC);
		if (pPkt == NULL)
			return FALSE;
...
{
	if (RTMP_OS_Alloc_Rsc(pAtomicList, pAtomic,
							sizeof(atomic_t)) == FALSE)
	{
