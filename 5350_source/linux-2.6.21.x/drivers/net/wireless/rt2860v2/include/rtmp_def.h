...
/* ========================================================================== */
#define RT_VLAN_8023_HEADER_COPY(__pAd, __pHeader8023, __HdrLen, __pData,	\
								__FromWhichBSSID, __TPID)					\
{																			\
	USHORT __VLAN_VID = 0, __VLAN_Priority = 0;								\
	...
	VLAN_8023_Header_Copy(__VLAN_VID, __VLAN_Priority,						\
				__pHeader8023, __HdrLen, __pData, __FromWhichBSSID,			\
				__TPID);													\
}


#define RT_VLAN_PKT_DUPLICATE(__pPacket, __pAd, __pHeader8023, __HdrLen,	\
							__pData, __DataSize, __FromWhichBSSID, __TPID)	\
{																			\
	USHORT __VLAN_VID = 0, __VLAN_Priority = 0;								\
	if ((__FromWhichBSSID < __pAd->ApCfg.BssidNum) &&						\
		(__pAd->ApCfg.MBSSID[__FromWhichBSSID].VLAN_VID != 0))				\
...
#ifdef CONFIG_AP_SUPPORT
#define RT_80211_TO_8023_PACKET(__pAd, __pRxBlk, __pHeader802_3,			\
								__FromWhichBSSID, __TPID)					\
{																			\
	USHORT __VLAN_VID = 0, __VLAN_Priority = 0;								\
	if ((__FromWhichBSSID < __pAd->ApCfg.BssidNum) &&						\
...
#define RTMP_L2_FRAME_TX_ACTION(__pAd, __ApIdx, __FrameBuf, __FrameLen)		\
	RTMPL2FrameTxAction(__pAd, get_netdev_from_bssid(__pAd, __ApIdx),		\
						announce_802_3_packet, __ApIdx, __FrameBuf, __FrameLen)
