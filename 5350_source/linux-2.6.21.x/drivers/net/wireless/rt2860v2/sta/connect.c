	...
	IN MLME_QUEUE_ELEM * Elem) 
{
	...
	{   
		...
		else
		{
			...
			if ( (pAd->MlmeAux.SsidBssTab.BssEntry[pAd->MlmeAux.BssIdx].Channel == 14) && 
				((pAd->CommonCfg.CountryRegion & 0x7f) == REGION_33_BG_BAND) &&
				(pAd->CommonCfg.SavedPhyMode == 0xFF ) )
			{
				pAd->CommonCfg.SavedPhyMode = pAd->CommonCfg.PhyMode;
				RTMPSetPhyMode(pAd, PHY_11B);
			}
			else if ( (pAd->MlmeAux.SsidBssTab.BssEntry[pAd->MlmeAux.BssIdx].Channel != 14) && 
				(pAd->MlmeAux.SsidBssTab.BssEntry[pAd->MlmeAux.BssIdx].Channel != 0) &&
			    	((pAd->CommonCfg.CountryRegion & 0x7f) == REGION_33_BG_BAND) &&
			     	(pAd->CommonCfg.SavedPhyMode != 0xFF ))
			{
