		...
		&CkipFlag)) 
	{
		if(MAC_ADDR_EQUAL(Addr2, pAd->MlmeAux.Bssid)) // The frame is for me ?
		{
			...
			if(Status == MLME_SUCCESS) 
			{
                ...
                if (pAd->StaCfg.WpaSupplicantUP != WPA_SUPPLICANT_DISABLE)
			{
				SendAssocIEsToWpaSupplicant(pAd->net_dev, pAd->StaCfg.ReqVarIEs, pAd->StaCfg.ReqVarIELen);
				RtmpOSWrielessEventSend(pAd->net_dev, RT_WLAN_EVENT_CUSTOM, RT_ASSOC_EVENT_FLAG, NULL, NULL, 0);
			}
#endif // NATIVE_WPA_SUPPLICANT_SUPPORT //
#endif // WPA_SUPPLICANT_SUPPORT //
...
#ifdef NATIVE_WPA_SUPPLICANT_SUPPORT
                {
                    wext_notify_event_assoc(pAd->net_dev, pAd->StaCfg.ReqVarIEs, pAd->StaCfg.ReqVarIELen);
                    RtmpOSWrielessEventSend(pAd->net_dev, RT_WLAN_EVENT_CGIWAP, -1, &pAd->MlmeAux.Bssid[0], NULL, 0);
                }
#endif // NATIVE_WPA_SUPPLICANT_SUPPORT //
...
//        union iwreq_data    wrqu;
		...
		RtmpOSWrielessEventSend(pAd->net_dev, RT_WLAN_EVENT_CGIWAP, -1, pAd->MlmeAux.Bssid, NULL, 0);
    }
#endif // NATIVE_WPA_SUPPLICANT_SUPPORT //
	return TRUE;
}
